...
uint16_t UserH::shooter_heat_log[2] = {0,0};
uint16_t UserH::heat_per_bullet = 0;
...
void UserH::UserThread::main() {
    ...
    while (!shouldTerminate()) {
        ...
        /// Update the heat_data
        shooter_heat_log[0] = shooter_heat_log[1];
        shooter_heat_log[1] = Referee::power_heat_data.shooter_heat1;

        if(shooter_heat_log[1] - shooter_heat_log[0] > 0 && shooter_heat_log[1] < Referee::game_robot_state.shooter_heat1_cooling_limit+ 400) {
            heat_per_bullet = shooter_heat_log[1] - shooter_heat_log[0];
        }
...
//            eventflags_t mouse_flag = chEvtGetAndClearFlags(&mouse_press_listener);
            ...
            if(heat_per_bullet + shooter_heat_log[1] < Referee::game_robot_state.shooter_heat1_cooling_limit){
                HeroShootLG::shoot();
            }
