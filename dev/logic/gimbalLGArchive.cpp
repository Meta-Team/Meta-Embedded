...
/// TODO: return AHRS angle
float GimbalLG::get_actual_angle(GimbalSKD::angle_id_t angleID) {
    return GimbalSKD::get_AHRS_angle(angleID);
}

float GimbalLG::get_relative_angle(GimbalSKD::angle_id_t angleID) {
    return GimbalSKD::get_relatvie_angle(angleID);
}

float GimbalLG::get_current_target_angle(GimbalSKD::angle_id_t angleID) {
    return GimbalSKD::get_target_angle(angleID);
}

void GimbalLG::separate_pitch() {
    sub_pitch_to_ground =   GimbalSKD::get_relatvie_angle(GimbalSKD::PITCH) -
                            GimbalSKD::get_relatvie_angle(GimbalSKD::SUB_PITCH);
}

void GimbalLG::cal_separate_angle(float &target_pitch, float &target_sub_pitch) {
    float temp_target_pitch = sub_pitch_to_ground;
    float flight_time;
    bool ret = Trajectory::compensate_for_gravity(temp_target_pitch, LidarIF::dist, 10, flight_time);
    if (ret) {
        target_pitch = temp_target_pitch;
        target_sub_pitch = GimbalLG::get_actual_angle(GimbalSKD::PITCH) - sub_pitch_to_ground;
    }
}
