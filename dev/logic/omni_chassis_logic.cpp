...
void ChassisLG::set_mode(ChassisLG::chassis_mode_t mode) {
    ...
    switch (mode) {
        case FORCE_RELAX_MODE:
            OmniChassisSKD::set_mode(SKDBase::FORCED_RELAX_MODE);
            break;
        case CHASSIS_REF_MODE:
            OmniChassisSKD::set_mode(SKDBase::CHASSIS_REF_MODE);
            target_omega = 0;
            break;
#if ENABLE_AHRS
    ...
    while(!shouldTerminate()) {
        ...
        switch (chassis_mode) {
            case FORCE_RELAX_MODE:
                break;
            case CHASSIS_REF_MODE:
                OmniChassisSKD::set_target(target_vx, target_vy, target_omega);
                break;
#if ENABLE_AHRS
                case GIMBAL_REF_MODE:
