...
void chasisPidController::chasis_pid_calc(float measured_velocity_X, float measured_velocity_Y,
     ...
     */
    float real_angular_velocity = (measured_angle - chasis_angle) / max_ang * 180 / chassis_thread_interval * 1000.0f;
    output[0] = calc(measured_velocity_Y + measured_velocity_X - real_angular_velocity * front_rotate_ratio,
                                                                target_velocity_Y + target_velocity_X - target_angular_velocity * front_rotate_ratio)*front_left_current_ratio*CHASSIS_WHEEL_RATIO;

    output[1] = calc(measured_velocity_Y - measured_velocity_X + real_angular_velocity * front_rotate_ratio,
                                                                 target_velocity_Y - target_velocity_X + target_angular_velocity * front_rotate_ratio)*front_right_current_ratio*CHASSIS_WHEEL_RATIO;

    output[2] = calc(measured_velocity_Y - measured_velocity_X  - real_angular_velocity * back_rotate_ratio,
                                                               target_velocity_Y - target_velocity_X - target_angular_velocity *back_rotate_ratio)*back_left_current_ratio*CHASSIS_WHEEL_RATIO;

    output[3] = calc(measured_velocity_Y + measured_velocity_X + real_angular_velocity * back_rotate_ratio,
                                                                target_velocity_Y + target_velocity_X + target_angleule_velocity * back_rotate_ratio)*back_right_current_ratio*CHASSIS_WHEEL_RATIO;

    chasis_angle = measured_angle;
}

void chasisPidController::chasis_pid_set_front(float measured_angle) {
    /**
     * @brief   set front ang
     */
...
float chasisPidController::read_front_angle() {
    /**
