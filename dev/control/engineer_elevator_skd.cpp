...
void EngineerElevatorSKD::update_target_current() {
    if (elevator_enabled){
        ...
        EngineerElevatorIF::elevatorMotor[0].target_current = (int16_t) v2i_pid[0].calc(EngineerElevatorIF::elevatorMotor[0].actual_velocity,
                a2v_pid[0].calc(angle_0, target_height * ANGLE_HEIGHT_RATIO));
        EngineerElevatorIF::elevatorMotor[1].target_current = (int16_t) v2i_pid[1].calc(EngineerElevatorIF::elevatorMotor[1].actual_velocity,
                a2v_pid[1].calc(angle_1, target_height * ANGLE_HEIGHT_RATIO) + counter_balance_pid.calc(angle_1, angle_0));
    } else{
