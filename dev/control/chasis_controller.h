...
private:
    ...
    float chasis_angle;    //chasis_angle with respect to front direction
public:
    float output[4];
    chasisPidController():PIDController() {}

    chasisPidController(float kp_v_loop, float ki_v_loop, float kd_v_loop, float i_limti,
        ...
        chasis_pid_set_front(current_angle);
        chasis_angle = current_angle;
    }
    void chasis_pid_change_parameters(float kp_v_loop, float ki_v_loop, float kd_v_loop, float i_limit,float out_limit);
    void chasis_pid_calc(float measured_velocity_X, float measured_velocity_Y, float measured_angle, float target_velocity_X,
                                 float target_velocity_Y, float target_angular_velocity);
    void chasis_pid_set_front(float measured_angle);
